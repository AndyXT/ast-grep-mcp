...
def generate_handler(language_name: str) -> None:
    ...
    if language_name not in LANGUAGE_CONFIG:
        print(
            f"Error: Language '{language_name}' is not currently supported by this script."
        )
        print(f"Supported languages: {', '.join(LANGUAGE_CONFIG.keys())}")
        print(
            "You can still create a handler manually by adding the necessary configuration."
        )
        sys.exit(1)
    ...
    if handler_file.exists():
        print(
            f"Error: Handler file for {language_name} already exists at {handler_file}"
        )
        sys.exit(1)
    ...
    if init_file.exists():
        ...
        if f"{class_name}Handler" in init_content:
            ...
            if match:
                ...
                import_line = (
                    f"from .{language_name}_handler import {class_name}Handler"
                )
