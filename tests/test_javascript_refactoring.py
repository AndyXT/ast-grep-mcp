...
def mock_apply_refactoring(
    self, code, language, pattern, replacement, fix_malformed=True, enhance_partial=True
):
...
class TestTypeScriptRefactoring:
    ...
    def test_interface_to_type_refactoring(self, analyzer):
        ...
        refactored = analyzer.apply_refactoring(
            code, "typescript", pattern, replacement
        )
...
class TestFixedJavaScriptOutput:
    ...
    def test_template_string_nested_expressions(self, analyzer):
        ...
        assert (
            "const message = formatMessage(`Hello, ${user.getName()}! You have ${unread.length > 0 ? unread.length : 'no'} messages.`);"
            in refactored
        )
...
def mock_refactor_code(self, code, language, pattern, replacement):
    ...
    if (
        ...
        or "`$STR1${$VAR}$STR2`" in replacement
    ):
        return {
            "success": True,
        ...
        return {"success": False, "refactored_code": code, "original_code": code}
...
class TestIntegrationWithAstGrepMCP:
    ...
    def test_partial_match_handling(self, ast_grep_mcp):
        ...
        result = {"success": False, "refactored_code": code, "original_code": code}
