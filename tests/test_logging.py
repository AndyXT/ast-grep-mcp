...
def test_logging_during_tool_execution(caplog):
    ...
    server.analyze_code(
        code="def test(): pass", language="python", pattern="def $FUNC_NAME(): $BODY"
    )
...
def test_log_format():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        config = ServerConfig(
            log_file=log_file, log_to_console=False, log_format=custom_format
        )
        server = AstGrepMCP(config)
...
def test_error_logging(server, caplog):
    ...
    # Test with an invalid language
    caplog.clear()
    server.analyze_code(code="def test(): pass", language="nonexistent", pattern="test")
