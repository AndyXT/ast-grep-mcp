...
class TestSecurityIntegration:
    ...
    def test_pattern_sanitization_in_analyze_code(self):
        ...
        dangerous_pattern = "print`evil command`($MSG)"
    ...
    def test_pattern_sanitization_in_refactor_code(self):
        ...
        result = instance.refactor_code(
            code, "python", dangerous_pattern, dangerous_replacement
        )
    ...
    def test_file_access_restrictions(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config = ServerConfig(log_to_console=False, safe_roots=[safe_dir])
            instance = AstGrepMCP(config)

            # Test access to safe file
            safe_result = instance.analyze_file(safe_file, "print($MSG)")
            assert "error" not in safe_result
            assert safe_result["count"] == 1

            # Test access to unsafe file
            unsafe_result = instance.analyze_file(unsafe_file, "print($MSG)")
