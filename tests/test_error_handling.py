...
def ast_grep_mcp():
    ...
    with patch("src.ast_grep_mcp.core.ast_grep_mcp.FastMCP") as mock_fastmcp:
        ...
        mock_mcp_instance.tool.return_value = (
            lambda x: x
        )  # The decorator should just return the original function
...
def test_get_language_patterns_with_exception(ast_grep_mcp):
    ...
    with patch(
        ...
        mock_handle_errors.side_effect = lambda func: (
            lambda *args, **kwargs: (
                {"error": "Exception: Test error", "patterns": {}}
                if "python" in args
                else func(*args, **kwargs)
            )
        )
        ...
        # Apply the decorator
        ast_grep_mcp.get_language_patterns = mock_handle_errors(
            ast_grep_mcp.get_language_patterns
        )

        # Patch get_handler to avoid errors
        with patch(
            "src.ast_grep_mcp.core.ast_grep_mcp.get_handler"
        ) as mock_get_handler:
...
def test_get_supported_languages_with_exception(ast_grep_mcp):
    ...
    with patch(
        ...
        # Set up the decorator to actually work
        mock_handle_errors.side_effect = lambda func: (
            lambda *args, **kwargs: {"error": "Exception: Test error", "languages": {}}
        )

        # Apply the decorator
        ast_grep_mcp.get_supported_languages = mock_handle_errors(
            ast_grep_mcp.get_supported_languages
        )
