from src.ast_grep_mcp.language_handlers.javascript_handler import (
    JavaScriptHandler,
    TypeScriptHandler,
)
...
class TestJavaScriptHandler:
    ...
    def test_get_default_patterns(self):
        ...
        performance_patterns = [
            "array_push_in_loop",
            "inefficient_dom_query",
            "redundant_jquery_selector",
            "innerHTML_in_loop",
...
class TestTypeScriptHandler:
    ...
    def test_get_default_patterns(self):
        ...
        assert (
            len(patterns) >= 40
        ), f"Expected at least 40 patterns, got {len(patterns)}"
        ...
        for pattern in ts_refactoring:
            assert (
                pattern in patterns
            ), f"Missing TypeScript refactoring pattern: {pattern}"
        ...
        for pattern in js_patterns:
            assert (
                pattern in patterns
            ), f"Missing JavaScript pattern in TypeScript handler: {pattern}"
