...
def analyze_file(
    file_path: str, analyzer: AstAnalyzer, patterns: Dict[str, str], category: str
) -> List[Dict[str, Any]]:
                    ...
                    results.append(
                        {
                            ...
                            "column": match["location"]["start"]["column"]
                            + 1,  # 1-based column numbers
                            "text": match["text"],
                        }
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="AST Grep Analyzer for Python codebase"
    )
    parser.add_argument("--directory", default="src", help="Directory to analyze")
    parser.add_argument(
        "--output", default="ast-grep-analysis.json", help="Output file path"
    )
