...
class Benchmark:
    ...
    def run_benchmark(self, label: str, func: Callable, *args, **kwargs) -> Tuple[List[float], List[Any]]:
        """
...
def create_synthetic_files(
    ...
    for i in range(num_files):
        ...
        for _ in range(templates_per_file):
            ...
            content += template.format(
                ...
                condition=random.choice(["True", "False"]) if language == "python" else random.choice(["true", "false"]),
    ...
    if num_files >= 20:
        ...
        for i in range(min(5, num_files // 20)):
            ...
            complex_templates = python_templates["complex"] if language == "python" else js_templates["complex"]
            for _ in range(lines // 20):
                ...
                content += template.format(
                    ...
                    params=", ".join([f"param{j}" for j in range(random.randint(1, 7))]),
...
def run_benchmarks(
    ...
    logger.info(f"Best overall configuration: {best_config_name} with {best_avg_speedup:.2f}x speedup")
