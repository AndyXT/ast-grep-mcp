...
def sanitize_pattern(pattern: str) -> str:
    ...
    if pattern != original:
        logger.warning(f"Potentially dangerous pattern detected and sanitized: {original[:50]} ->")
...
def validate_file_access(path: str, safe_roots: Optional[List[str]] = None) -> Optional[str]:
    """
