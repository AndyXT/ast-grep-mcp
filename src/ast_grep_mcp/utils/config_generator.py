...
def generate_commented_config() -> str:
    ...
    lines.append(
        f"use_default_ignores: {str(config_dict['use_default_ignores']).lower()}"
    )
    ...
    if pattern_config["custom_patterns"]:
        ...
        for lang, patterns in pattern_config["custom_patterns"].items():
            ...
            lines.extend(
                [f"      {name}: '{pattern}'" for name, pattern in patterns.items()]
            )
    else:
        lines.append("    # python:")
    ...
    lines.append(
        f"  validate_replacements: {str(refactoring_config['validate_replacements']).lower()}"
    )
    lines.append(f"  {comments['max_replacements']}")
    lines.append(
        f"  max_replacements: {refactoring_config['max_replacements'] or 'null'}"
    )
    lines.append(f"  {comments['fix_malformed_output']}")
    lines.append(
        f"  fix_malformed_output: {str(refactoring_config['fix_malformed_output']).lower()}"
    )
    lines.append(f"  {comments['enhance_partial_matches']}")
    lines.append(
        f"  enhance_partial_matches: {str(refactoring_config['enhance_partial_matches']).lower()}"
    )
    lines.append("")
    ...
    lines.append(
        f"  show_line_numbers: {str(output_config['show_line_numbers']).lower()}"
    )
    ...
    lines.append(
        f"  pattern_diagnostics: {str(diagnostic_config['pattern_diagnostics']).lower()}"
    )
    lines.append(f"  {comments['show_suggestions']}")
    lines.append(
        f"  show_suggestions: {str(diagnostic_config['show_suggestions']).lower()}"
    )
    ...
    lines.append(
        f"  show_stack_traces: {str(diagnostic_config['show_stack_traces']).lower()}"
    )
    lines.append("")
    ...
    if config_dict["options"]:
        lines.extend(
            [f"  {name}: {value}" for name, value in config_dict["options"].items()]
        )
    else:
