...
def is_pattern_syntax_error(error_message: str) -> bool:
    ...
    return any(indicator in error_message.lower() for indicator in syntax_error_indicators)
...
def analyze_pattern_error(pattern: str, language: str) -> Dict[str, Any]:
    ...
    error_checks = [
        (lambda p: p.count('(') != p.count(')'), "mismatched_brackets", "Mismatched parentheses ()"),
        (lambda p: p.count('[') != p.count(']'), "mismatched_brackets", "Mismatched brackets []"),
        (lambda p: p.count('{') != p.count('}'), "mismatched_brackets", "Mismatched braces {}"),
        (lambda p: p.count('<') != p.count('>') and language in ['javascript', 'typescript'], "mismatched_brackets", "Mismatched angle brackets <>"),
        (lambda p: re.search(r'\$\$[^$\w]', p), "invalid_variable", "Invalid metavariable (should be $ or $$$)"),
        (lambda p: '$$' in p and '$$$' not in p, "invalid_variable", "Invalid metavariable (use $$$ for variadic)"),
        (lambda p: re.search(r'\$\s+\w', p), "invalid_variable", "Space after $ in metavariable"),
        (lambda p: re.search(r'\$\w*\d+\w*', p) and language not in ['rust'], "invalid_variable", "Numbers in metavariable names"),
        (lambda p: '`' in p and ('${' not in p and language in ['javascript', 'typescript']), "template_literal", "Template literal without ${} interpolation"),
        (lambda p: '${' in p and '`' not in p and language in ['javascript', 'typescript'], "template_literal", "Template syntax outside backticks"),
    ]
    ...
    for check_func, error_type, message in error_checks:
        if check_func(pattern):
            ...
            result["errors"].append({
                "type": error_type,
                "message": message,
                "solution": COMMON_SYNTAX_ERRORS.get(error_type, "Check syntax documentation")
            })
    ...
    if language in LANGUAGE_SPECIFIC_ERRORS:
        ...
        if language == "python":
            if ":" in pattern and not re.search(r'(if|for|while|def|class|with|try|except|lambda).*:', pattern):
                result["language_specific"].append(lang_errors["missing_colon"])
            
            if "\n" in pattern and re.search(r'\n\s+\S', pattern) and not re.search(r'\n\s{4}\S', pattern):
                result["language_specific"].append(lang_errors["inconsistent_indentation"])
        
        # JavaScript/TypeScript specific checks
        elif language in ["javascript", "typescript"]:
            if "<" in pattern and ">" in pattern and not re.search(r'<\/?[A-Za-z]([^<>]*)(\/?)>', pattern):
                result["language_specific"].append(lang_errors["jsx_syntax"])
                
            if "=>" in pattern and not re.search(r'(\(.*\)|[a-zA-Z_$][0-9a-zA-Z_$]*)\s*=>\s*(\{.*\}|[^{])', pattern):
...
def get_pattern_help(language: str, error_message: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    if error_message and is_pattern_syntax_error(error_message):
        error_analysis = analyze_pattern_error("", language)  # Analyze empty pattern as fallback
