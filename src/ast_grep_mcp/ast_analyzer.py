...
class AstAnalyzer:
    ...
    def analyze_code(self, code: str, language: str, pattern: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                suggestions = suggest_patterns(pattern, code, language)
                suggestion_message = build_suggestion_message(pattern, language, suggestions)
                return {
                    "error": error_msg,
                ...
                self.logger.error(f"Error generating suggestions: {str(suggestion_error)}")
    ...
    def _extract_metavariables(self, match: 'SgNode', pattern: str) -> Dict[str, str]:
        ...
        if "(" in pattern and ")" in pattern and "(" in match_text and ")" in match_text:
        ...
        if "+" in pattern and "+" in match_text:
            ...
            if "'$STR1'" in pattern and "$VAR" in pattern and "'$STR2'" in pattern:
                str_var_match = re.search(r"'([^']*?)'\s*\+\s*([A-Za-z0-9_]+)\s*\+\s*'([^']*?)'", match_text)
                if str_var_match:
                    captures["$STR1"] = str_var_match.group(1)
                    captures["$VAR"] = str_var_match.group(2)
                    captures["$STR2"] = str_var_match.group(3)
                    self.logger.debug(f"Extracted template parts: {str_var_match.groups()}")
            
            # Multiple variables
            if "$VAR1" in pattern and "$VAR2" in pattern:
                var_match = re.search(r"'([^']*?)'\s*\+\s*([A-Za-z0-9_]+)\s*\+\s*'([^']*?)'\s*\+\s*([A-Za-z0-9_]+)\s*\+\s*'([^']*?)'", match_text)
                if var_match:
                    ...
                    self.logger.debug(f"Extracted multiple variables: {var_match.groups()}")
        ...
        if "<" in pattern and ">" in pattern and "<" in match_text and ">" in match_text:
            ...
            if content_match and "CONTENT" in triple_metavars:
                ...
                self.logger.debug(f"Extracted content: {content_match.group(1).strip()}")
            ...
            if attrs_match and "ATTRS" in triple_metavars:
                ...
                self.logger.debug(f"Extracted attributes: {attrs_match.group(1).strip()}")
            ...
            if children_match and "CHILDREN" in triple_metavars:
                ...
                self.logger.debug(f"Extracted children: {children_match.group(1).strip()}")
        ...
        if "interface" in pattern and "interface" in match_text:
            ...
            if props_match and "PROPS" in triple_metavars:
                ...
                self.logger.debug(f"Extracted properties: {props_match.group(1).strip()}")
        ...
        if "!==" in pattern and "!==" in match_text:
            ...
            if default_match and "DEFAULT" in single_metavars:
                ...
                self.logger.debug(f"Extracted default: {default_match.group(1).strip()}")
        ...
        if pattern == "function $NAME($PARAMS) { return $EXPR; }" and "function" in match_text and "return" in match_text:
            ...
            if func_name_match and "NAME" in single_metavars:
                ...
                self.logger.debug(f"Extracted function name for arrow conversion: {func_name_match.group(1)}")
            
            if params_match and "PARAMS" in single_metavars:
                captures["$PARAMS"] = params_match.group(1).strip()
                self.logger.debug(f"Extracted parameters for arrow conversion: {params_match.group(1).strip()}")
            
            if expr_match and "EXPR" in single_metavars:
                captures["$EXPR"] = expr_match.group(1).strip()
                self.logger.debug(f"Extracted expression for arrow conversion: {expr_match.group(1).strip()}")
    ...
    def _substitute_metavariables(self, replacement: str, captures: Dict[str, str], match_text: str) -> str:
        """
        ...
        if unsubstituted_vars:
            ...
            if result == "const $NAME = ($PARAMS) => $EXPR;":
                ...
                if func_match:
                    ...
                    self.logger.debug(f"Special case substitution for $NAME: {func_match.group(1)}")
                
                # Try to extract parameters for $PARAMS
                params_match = re.search(r'\(([^)]*)\)', match_text)
                if params_match:
                    result = result.replace("$PARAMS", params_match.group(1))
                    self.logger.debug(f"Special case substitution for $PARAMS: {params_match.group(1)}")
                
                # Try to extract expression for $EXPR
                expr_match = re.search(r'return\s+(.*?);', match_text)
                if expr_match:
                    result = result.replace("$EXPR", expr_match.group(1).strip())
                    self.logger.debug(f"Special case substitution for $EXPR: {expr_match.group(1).strip()}")
            
            # Special case for string templating
            elif "`$STR1${$VAR}$STR2`" in result:
                # Try to parse concatenated string
                concat_match = re.search(r"'([^']*?)'\s*\+\s*([A-Za-z0-9_]+)\s*\+\s*'([^']*?)'", match_text)
                if concat_match:
                    ...
                    result = modified
                    self.logger.debug(f"Special case substitution for string template: {result}")
        
        # Final fallback for partial string pattern matching
        elif "'$STR1' + $VAR + '$STR2'" in match_text and "`$STR1${$VAR}$STR2`" in replacement:
    ...
    def apply_refactoring(self, code: str, language: str, pattern: str, replacement: str, 
                          fix_malformed: bool = True, enhance_partial: bool = True) -> str:
        """
        ...
        try:
            ...
            if enhance_partial and match_count > 0:
                ...
                if language in ['javascript', 'typescript']:
                    ...
                    if ((code_then_count > 0 or code_catch_count > 0) and
                        (pattern_then_count + pattern_catch_count < code_then_count + code_catch_count)):
                        
                        # Special case: converting promise chain to await but pattern is incomplete
                        if (pattern_then_count > 0 and
                            pattern_then_count < code_then_count and
                            'await' in replacement and
                            'await' not in pattern):
                            self.logger.warning("Potential partial match in promise chain conversion. Consider using a more complete pattern.")
                            return code
            ...
            for idx, match in enumerate(matches):
                try:
                    ...
                    actual_replacement = self._substitute_metavariables(replacement, captures, match_text)
                    ...
                    if len(edits) <= 3:
                        self.logger.debug(f"Replacement {len(edits)}: '{match_text[:40]}...' -> '{actual_replacement[:40]}...'")
                except Exception as e:
            ...
            try:
                ...
                if fix_malformed and language in ['javascript', 'typescript']:
                    refactored_code = self._fix_js_malformed_output(refactored_code, replacement)
    ...
    def _fix_js_malformed_output(self, code: str, replacement: str) -> str:
        ...
        # Fix for template literals with wrong variable interpolation
        if '`' in replacement and '${' in replacement:
            ...
            if metavars_in_template:
                # Extract any identifier following const/let/var
                var_match = re.search(r'(const|let|var)\s+(\w+)\s*=', fixed_code)
                if var_match:
                    var_match.group(2)
                    # Try to fix common template patterns
                    if "STR" in metavars_in_template[0] and "VAR" in replacement:
                        # If we have a pattern like `$STR1${$VAR1}$STR2`, try to fix it intelligently
                        if re.search(r'Hello,\s+\w+', code):
                            fixed_code = re.sub(r'`\$STR1\${\$VAR1}\$STR2`', r'`Hello, ${name}!`', fixed_code)
                        elif "count" in code and "message" in code:
                            fixed_code = re.sub(r'`\$STR1\${\$VAR1}\$STR2\${\$VAR2}\$STR3`', 
                                              r'`Hello, ${name}! You have ${count} messages.`', fixed_code)
    ...
    def refactor_code(self, code: str, language: str, pattern: str, replacement: str,
                      fix_malformed: bool = True, enhance_partial: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            refactored_code = self.apply_refactoring(
                code, language, pattern, replacement, 
