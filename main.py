...
from src.ast_grep_mcp.utils.config_generator import generate_and_save_config, generate_example_ignore_file
import sys
import logging
import tempfile
import json
...
@app.command()
def generate_config(
    output_file: str = typer.Option("ast-grep.yml", "--output", "-o", help="Output file path"),
    ignore_file: bool = typer.Option(False, "--ignore-file", "-i", help="Also generate example .ast-grepignore file"),
    ignore_output: str = typer.Option(".ast-grepignore", help="Path for the generated ignore file"),
    ...
    try:
        ...
        console.print(f"[bold green]Generated configuration file:[/bold green] {output_file}")
        
        # Generate ignore file if requested
        if ignore_file:
            if os.path.exists(ignore_output) and not force:
                console.print(f"[bold red]File already exists:[/bold red] {ignore_output}")
                console.print("Use --force to overwrite")
            else:
                generate_example_ignore_file(ignore_output)
                console.print(f"[bold green]Generated ignore file:[/bold green] {ignore_output}")
        
        console.print("\n[bold blue]Next steps:[/bold blue]")
        console.print("1. Review and customize the generated configuration")
        console.print("2. Use the configuration with 'ast-grep-mcp start --config ast-grep.yml'")
...
def start(
    ...
    ignore_file: str = typer.Option(None, "--ignore-file", 
                                   help="Path to .ast-grepignore file"),
    preview_mode: bool = typer.Option(False, "--preview-mode", 
                                     help="Enable refactoring preview mode"),
    validation_strictness: str = typer.Option(None, "--validation-strictness",
    ...
    output_format: str = typer.Option(None, "--output-format",
                                   help="Output format (json, text, sarif, html)"),
):
    """
    ...
    try:
        ...
        if config_file is None:
            ...
            console.print(f"Using configuration from: {config_file if config_file else 'default settings'}")
        else:
        ...
        if safe_roots is not None:
            overrides["safe_roots"] = [root.strip() for root in safe_roots.split(",") if root.strip()]
...
def serve(
    ...
    return start(host, port, log_level, log_file, log_to_console, cache_size, config_file, safe_roots,
                ignore_file, preview_mode, validation_strictness, verbosity, output_format)
...
def pattern_builder():
    ...
    def show_language_selection():
        ...
        while True:
            try:
                choice = input("Select language number (or press Enter to keep current): ")
                if not choice.strip():
...
""",
                ...
                current_code = examples.get(current_language, "// No example available for this language")
            else:
    ...
    def show_pattern_help():
        ...
        handler = get_handler(current_language)
        if not handler:
            console.print("[bold yellow]No pattern examples available for this language[/bold yellow]")
            return
        ...
        for name, pattern in patterns.items():
            ...
            if name.startswith("function_"):
                ...
                category = "Classes"
            elif name.startswith("if_") or name.startswith("for_") or name.startswith("while_"):
    ...
    def test_pattern(pattern, show_results=True):
        ...
        if not current_code:
            console.print("[bold yellow]No code loaded. Use 'load' to load code first.[/bold yellow]")
            return None
        ...
        if show_results:
            ...
            for i, match in enumerate(matches[:5], 1):  # Show at most 5 matches
                ...
                console.print(f"  Line: {match['location']['start']['line']} - {match['location']['end']['line']}")
    ...
    def load_user_patterns():
        ...
        if not pattern_file.exists():
            console.print(f"[bold yellow]No saved patterns found for {current_language}[/bold yellow]")
            return []
    ...
    def interactive_builder():
        ...
        while True:
            ...
            if choice == "1":
                ...
                print("  1. Make a variable more specific (e.g., $VAR → $SPECIFIC_NAME)")
                ...
                if refine_choice == "1":
                    ...
                    if not var_matches:
                        console.print("[bold yellow]No variables found in pattern[/bold yellow]")
                        continue
                    ...
                    try:
                        ...
                        if 0 <= index < len(var_matches):
                            ...
                            if new_name:
                                current_pattern = current_pattern[:var_match.start()] + "$" + new_name + current_pattern[var_match.end():]
                        else:
                            console.print("[bold red]Invalid selection[/bold red]")
                    except ValueError:
                        console.print("[bold red]Please enter a number[/bold red]")
                        ...
                        if 0 <= index < len(var_matches):
                            ...
                            try:
                                ...
                                if 0 <= gen_index < len(generic_options):
                                    current_pattern = current_pattern[:var_match.start()] + "$" + generic_options[gen_index] + current_pattern[var_match.end():]
                                else:
                                    console.print("[bold red]Invalid selection[/bold red]")
                            except ValueError:
                                console.print("[bold red]Please enter a number[/bold red]")
                    ...
                    if not matches:
                        console.print("[bold yellow]No matches to add context around[/bold yellow]")
                        continue
                    ...
                    include_lines = input("Enter line numbers to include (e.g., 10-12): ")
                    try:
                        if '-' in include_lines:
                        ...
                        context_code = context_code.replace(match['text'], current_pattern)
                    ...
                    if '\n' in current_pattern:
                        ...
                        remove_lines = input("Enter line numbers to remove (e.g., 1,3): ")
                        try:
                            line_nums = list(map(int, remove_lines.split(',')))
                            kept_lines = [line for i, line in enumerate(lines, 1) if i not in line_nums]
                            current_pattern = '\n'.join(kept_lines)
                        except ValueError:
                            console.print("[bold red]Invalid line numbers[/bold red]")
                    else:
                        console.print("[bold yellow]Pattern is a single line, can't remove context[/bold yellow]")
                ...
                if matches:
                    ...
                    console.print("[bold yellow]Pattern has no matches, save anyway? (y/n)[/bold yellow]")
                    if input().lower() == 'y':
                        save_pattern(current_pattern)
                        pattern_history.append(current_pattern)
                break
    ...
    if not current_code:
        console.print("[bold yellow]No code loaded. You can load code later with the 'load' command.[/bold yellow]")
        ...
        try:
            ...
            if command == "save":
                if not current_pattern:
                    console.print("[bold yellow]No current pattern to save[/bold yellow]")
                    continue
